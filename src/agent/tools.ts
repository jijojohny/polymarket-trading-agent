...
import type { SafetyManager } from './safety'
import {
  searchMarkets,
  getMarketBySlug,
...
async function resolveMarket(slugOrId: string): Promise<GammaMarket | null> {
  const cached = marketCache.get(slugOrId)
  if (cached) return cached

  // Try slug first, then conditionId
  let market = await getMarketBySlug(slugOrId)
  if (!market) market = await getMarketByConditionId(slugOrId)
  if (market) marketCache.set(slugOrId, market)
  return market
}
...
export async function executeTool(
  ...
  try {
    switch (name) {
      case 'search_markets': {
        const markets = await searchMarkets(String(input.query || ''), Number(input.limit) || 10)
        if (markets.length === 0) return 'No markets found matching that query.'
        return JSON.stringify(markets.map(formatMarketSummary), null, 2)
      }

      case 'get_top_markets': {
        const markets = await getTopMarkets(Number(input.limit) || 20)
        return JSON.stringify(markets.map(formatMarketSummary), null, 2)
      }

      case 'get_market': {
        const market = await resolveMarket(String(input.slug_or_id))
        if (!market) return `Market not found: ${input.slug_or_id}`
      ...
      case 'get_order_book': {
        ...
        if (!tokenId) return 'This market does not have CLOB trading enabled.'
      ...
      case 'get_open_orders': {
        ...
        if (!orders || (orders as unknown[]).length === 0) return 'No open orders.'
      ...
      case 'place_order': {
        const market = await resolveMarket(String(input.slug_or_id))
        if (!market) return `Market not found: ${input.slug_or_id}`
        if (!market.enableOrderBook) return 'This market does not support CLOB trading.'
        if (market.closed) return 'This market is closed.'
        if (!market.active) return 'This market is not active.'

        const tokenId = input.outcome === 'NO' ? market.clobTokenIds[1] : market.clobTokenIds[0]
        if (!tokenId) return 'Token ID not found for this market/outcome.'

        const sizeUsdc = Number(input.size_usdc)
        const orderType = String(input.order_type || 'LIMIT') as 'LIMIT' | 'MARKET'
        const price = input.price ? Number(input.price) : undefined

        // Safety check
        const safetyError = safety.checkOrder({ sizeUsdc })
        if (safetyError) return `ORDER BLOCKED: ${safetyError}`
      ...
      case 'get_trade_history': {
        ...
        if (!trades || (trades as unknown[]).length === 0) return 'No trade history found.'
...
function formatMarketSummary(m: GammaMarket) {
  return {
    slug: m.slug,
...
function formatMarketDetail(m: GammaMarket) {
  return {
