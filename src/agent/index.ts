import type { AIProvider, AIMessage } from '../ai/types'
...
export class Agent {
  ...
  constructor(ai: AIProvider, client: PolymarketClient, safety: SafetyManager) {
    this.ai = ai
    this.client = client
    this.safety = safety
  }
   ...
   */
  async chat(userMessage: string, onStep?: (msg: string) => void): Promise<string> {
    ...
    while (rounds < this.maxToolRounds) {
      rounds++
      const response = await this.ai.chat(messages, TOOL_DEFINITIONS)
      ...
      for (const tc of response.toolCalls) {
        ...
        const result = await executeTool(tc.name, tc.input, this.client, this.safety)
   ...
   */
  async autonomousScan(onStep?: (msg: string) => void): Promise<string> {
    const prompt = `
